...
import { Button, Card, Text, TextInput, useTheme, Divider } from "react-native-paper";
...
export default function UserSystemAccessTokenRequests() {
  ...
  const hasPendingRequest = useMemo(() => {
    return requests.some((r) => r.status === SystemAccessTokenRequestStatus.Pending);
  }, [requests]);
    ...
    <>
      <Card style={styles.card}>
        <Card.Content>
          ...
          {hasPendingRequest && (
            <View style={[styles.pendingWarning, { backgroundColor: theme.colors.surfaceVariant }]}>
              <Text variant="bodySmall" style={{ color: theme.colors.onSurfaceVariant }}>
                {t("systemAccessTokens.userRequests.pendingRequestWarning" as any)}
              </Text>
            </View>
          )}
          ...
          <View style={styles.list}>
            {requests.map((r: SystemAccessTokenRequestFragment) => (
              <View key={r.id} style={[styles.itemContainer, { backgroundColor: theme.colors.surface, borderColor: theme.colors.outlineVariant }]}>
                <View style={styles.itemHeader}>
                  ...
                  <Text
                    variant="bodySmall"
                    style={{
                      color: r.status === SystemAccessTokenRequestStatus.Approved
                ...
                <View style={styles.itemDetails}>
                  <Text variant="bodySmall" style={{ color: theme.colors.onSurfaceVariant }}>
                    {t("systemAccessTokens.item.created")}: {formatDate(r.createdAt)}
                  </Text>
                  <Text variant="bodySmall" style={{ color: theme.colors.onSurfaceVariant }}>
                    {t("systemAccessTokens.userRequests.maxRequests")}: {r.maxRequests}
                  </Text>
                  {r.systemAccessToken && (
                    <Text variant="bodySmall" style={{ color: theme.colors.onSurfaceVariant }}>
                      {t("systemAccessTokens.userRequests.tokenId" as any)}: {r.systemAccessToken.id}
                    </Text>
                  )}
                </View>
                  ...
                  <View style={styles.tokenSection}>
                    ...
                    <View style={styles.tokenRow}>
                      ...
                      <CopyButton
                        ...
                        label={t("systemAccessTokens.userRequests.copyToken" as any) as string}
                        successLabel={t("systemAccessTokens.userRequests.tokenCopied" as any) as string}
                      />
                    </View>
                  </View>
                )}
