...
public class KeychainAccess {
    ...
    if status == errSecSuccess {
      ...
      if key == "authToken", let item = result as? [String: Any] {
        ...
        if let accessToken = item[kSecAttrAccount as String] as? String {
          print(
            "🔑 [KeychainAccess] ✅ Token found in kSecAttrAccount (as String, length: \(accessToken.count))"
          )
          return accessToken
        }
        ...
        {
          print(
            "🔑 [KeychainAccess] ✅ Token found in kSecAttrAccount (as Data, length: \(accessToken.count))"
          )
          return accessToken
        } else {
  ...
  public static func decrementBadgeCount(source: String) {
    ...
    print(
      "🔑 [KeychainAccess] 🔢 Badge count decremented from \(currentCount) to \(newCount) (source: \(source))"
    )
  }
  ...
  public static func storeIntentInKeychain(data: [String: Any], service: String) throws {
    do {
      ...
      guard let jsonString = String(data: jsonData, encoding: .utf8) else {
        throw NSError(
          domain: "KeychainError", code: -1,
          userInfo: [NSLocalizedDescriptionKey: "Failed to convert intent to JSON string"])
      }

      let success = DatabaseAccess.setSettingValue(
        key: "auth_pendingNavigationIntent", value: jsonString)

      if success {
        print(
          "🔑 [KeychainAccess] ✅ Stored pending navigation intent in database (service: \(service))")
      } else {
        throw NSError(
          domain: "KeychainError", code: -1,
          userInfo: [NSLocalizedDescriptionKey: "Failed to store intent in database"])
      }
    } catch {
  ...
  public static func getIntentFromKeychain(service: String) -> [String: Any]? {
    guard let jsonString = DatabaseAccess.getSettingValue(key: "auth_pendingNavigationIntent"),
      ...
      print(
        "🔑 [KeychainAccess] ℹ️ No pending navigation intent found in database (service: \(service))")
      return nil
    }
    ...
    do {
      ...
      print(
        "🔑 [KeychainAccess] ✅ Retrieved pending navigation intent from database (service: \(service))"
      )
      return json
    } catch {
  ...
  public static func clearIntentFromKeychain(service: String) {
    ...
    if success {
      print(
        "🔑 [KeychainAccess] ✅ Cleared pending navigation intent from database (service: \(service))"
      )
    } else {
