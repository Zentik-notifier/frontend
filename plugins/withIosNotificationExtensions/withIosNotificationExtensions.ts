...
async function addAppExtensionTarget(
  ...
  if (existingTargetKey) {
    ...
    // Add any missing source files to the existing target
    ...
    for (const file of entries) {
      ...
      // Skip non-Swift files (except entitlements which are handled separately)
      ...
      // Check if file is already in the project for this target
      const fileRef = pbxProject.getFirstProject().firstProject.mainGroup;
      const existingBuildFiles = pbxProject.pbxBuildFileSection();
      const isFileAlreadyAdded = Object.values(existingBuildFiles).some(
        (bf: any) => {
          if (typeof bf === "string") return false;
          const fileRefObj = bf.fileRef_comment || "";
          return fileRefObj === file;
        },
      );

      if (!isFileAlreadyAdded) {
        console.log(`[${targetName}] ➕ Adding missing file to Xcode: ${file}`);
        const pbxGroupKey = pbxProject.pbxCreateGroup(targetName, targetName);
        pbxProject.addSourceFile(
          absPath,
          { target: existingTargetKey },
          pbxGroupKey,
        );
      }
    }
