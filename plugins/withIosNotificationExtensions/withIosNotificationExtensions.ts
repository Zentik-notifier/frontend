...
function copyDirSync(src: string, dest: string, mainBundleId?: string) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (
        mainBundleId &&
        (entry.name.endsWith(".swift") || entry.name.endsWith(".entitlements"))
      ) {
        ...
        if (
          ...
          const containerRegex =
            /(<key>com\.apple\.developer\.icloud-container-identifiers<\/key>\s*<array>\s*<string>iCloud\.)([^<]+)(<\/string>\s*<\/array>)/;
...
function copySharedFilesToTarget(
  ...
  if (!fs.existsSync(sharedFilesSource)) {
    console.log(
      `[${targetName}] ⚠️  Shared files directory not found: ${sharedFilesSource}`,
    );
    return;
  }

  // Files to exclude from extensions (React Native bridges and extension-specific stubs)
  const excludedFiles = [
    ...
    "WatchSyncManager.swift", // Extensions have their own stub (WC not available in extensions)
  ];

  // Read all .swift files from the ZentikShared directory
  const sharedFiles = fs
    .readdirSync(sharedFilesSource)
    .filter((file) => file.endsWith(".swift"))
    .filter((file) => !excludedFiles.includes(file));

  if (sharedFiles.length === 0) {
    console.log(
      `[${targetName}] ⚠️  No .swift files found in ${sharedFilesSource}`,
    );
    return;
  }

  console.log(
    `[${targetName}] 📦 Found ${sharedFiles.length} shared files to copy (${excludedFiles.length} excluded)`,
  );
  ...
  // Only remove CloudKit files that should NEVER be in extensions
  // Do NOT remove WatchSyncManager.swift - extensions have their own stub in the files/ folder
  const filesToRemove = [
    "CloudKitSyncBridge.swift",
    "CloudKitManager.swift",
    "WatchCloudKit.swift",
  ];
  for (const excludedFile of filesToRemove) {
...
async function addAppExtensionTarget(
  ...
  // Files to exclude from COPYING from ZentikShared (extensions have their own stubs)
  ...
  // Files to exclude from Xcode project (truly not needed)
  // Note: WatchSyncManager.swift is NOT here because extensions need the stub added to Xcode
  const excludedFromXcode = [
    "CloudKitSyncBridge.swift",
    "CloudKitManager.swift",
    "WatchCloudKit.swift",
  ];
  ...
  if (existingTargetKey) {
    console.log(
      `Target ${targetName} già presente, aggiornando file condivisi...`,
    );
  ...
  for (const file of entries) {
    // Skip excluded files for extensions (but NOT WatchSyncManager.swift - we need the stub)
    if (excludedFromXcode.includes(file)) {
      console.log(
        `[${targetName}] ⏭️  Skipping ${file} in Xcode project (excluded)`,
      );
      continue;
    }
  ...
  for (const fw of frameworksToLink) {
    ...
    pbxProject.addFramework(`System/Library/Frameworks/${fw}`, {
      target: target.uuid,
    });
  }
...
async function addCommunicationNotificationsCapability(
  ...
  try {
    ...
    if (!mainTarget) {
      console.log(
        `Target ${targetName} not found, skipping Communication Notifications capability`,
      );
      return;
    }
...
`;
      ...
      if (entitlements.includes("</dict>")) {
        entitlements = entitlements.replace(
          "</dict>",
          `${capabilityEntry}</dict>`,
        );
        fs.writeFileSync(entitlementsPath, entitlements, "utf8");
        console.log(
          `Added Communication Notifications capability to ${targetName}`,
        );
      }
    } else {
      console.log(
        `Communication Notifications capability already present in ${targetName}`,
      );
    }
  } catch (error) {
    console.log(
      `Error adding Communication Notifications capability: ${error}`,
    );
  }
}
...
const withZentikNotificationExtensions: ConfigPlugin = (config) => {
  return withXcodeProject(config, async (newConfig) => {
    ...
    const baseBundleId =
      config.ios?.bundleIdentifier ?? "com.apocaliss92.zentik";
